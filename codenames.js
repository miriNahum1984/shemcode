...
function scanQRCode() {
    ...
    try {
        ...
        if (code) {
            ...
            try {
                // פענוח קוד המשחק והטענתו לשדה הקלט
                const gameCodeInput = document.getElementById('gameCodeInput');
                if (gameCodeInput) {
                    gameCodeInput.value = code.data;

                    // הפעלת כפתור ההצטרפות באופן תכנותי
                    const joinGameBtn = document.getElementById('joinGameBtn');
                    if (joinGameBtn) {
                        joinGameBtn.click();
                    } else {
                        // אם לא מצאנו את הכפתור, ננסה להפעיל את הפונקציה ישירות
                        joinGame();
                    }
                } else {
                    throw new Error('לא נמצא שדה קלט לקוד המשחק');
                }
            } catch (error) {
...
function joinGame() {
    ...
    try {
        ...
        if (savedTurn) {
            ...
            console.log('נטען מצב תור מהאחסון המקומי:', savedTurn);
        } else {
            // קביעת התור לפי האות הראשונה בקוד
            gameState.blueTeamTurn = gameCode.charAt(0) === '1';
            // שמירת מצב התור בפעם הראשונה
            localStorage.setItem(`turn_${gameCode}`, gameState.blueTeamTurn ? 'blue' : 'red');
            console.log('נקבע תור התחלתי:', gameState.blueTeamTurn ? 'blue' : 'red');
        }
...
function initializeGame() {
    ...
    // שמירת מצב התור ההתחלתי באחסון המקומי
    localStorage.setItem(`turn_${gameState.boardCode}`, gameState.blueTeamTurn ? 'blue' : 'red');
    console.log('שמירת תור התחלתי באחסון:', gameState.blueTeamTurn ? 'blue' : 'red');
...
function renderBoard() {
    ...
    // ניסיון לבדוק אם יש עדכונים מאחסון מקומי
    if (gameState.boardCode) {
        const savedTurn = localStorage.getItem(`turn_${gameState.boardCode}`);
        if (savedTurn) {
            const shouldBeBlueTurn = savedTurn === 'blue';
            if (gameState.blueTeamTurn !== shouldBeBlueTurn) {
                console.log('סנכרון תור מהאחסון המקומי:', savedTurn);
                gameState.blueTeamTurn = shouldBeBlueTurn;
                updateTurnIndicator();
            }
        }

        const savedCards = localStorage.getItem(`revealed_${gameState.boardCode}`);
        if (savedCards) {
            const parsedCards = JSON.parse(savedCards);
            if (JSON.stringify(parsedCards) !== JSON.stringify(gameState.revealedCards)) {
                console.log('סנכרון קלפים חשופים מהאחסון המקומי');
                gameState.revealedCards = parsedCards;
            }
        }
    }
...
function revealCard(index) {
    ...
    // שמירת מצב התור הנוכחי לסנכרון בין מכשירים
    const currentTurn = gameState.blueTeamTurn ? 'blue' : 'red';
    localStorage.setItem(`turn_${gameState.boardCode}`, currentTurn);
    console.log('עדכון תור ב-localStorage:', currentTurn);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // כפתור רענון מצב המשחק
    document.getElementById('refreshGameBtn').addEventListener('click', () => {
        if (gameState.boardCode) {
            // טעינה מחדש של מצב המשחק מהאחסון המקומי
            const savedCards = localStorage.getItem(`revealed_${gameState.boardCode}`);
            const savedTurn = localStorage.getItem(`turn_${gameState.boardCode}`);

            if (savedCards) {
                gameState.revealedCards = JSON.parse(savedCards);
                console.log('נטענו קלפים חשופים מהאחסון המקומי');
            }
            ...
            alert('מצב המשחק רוענן בהצלחה!');
        } else {
            alert('אין משחק פעיל לרענון');
        }
    });
